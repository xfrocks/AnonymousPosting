...
// updated by DevHelper_Helper_ShippableHelper at 2016-08-13T17:42:56+00:00

/**
 * Class AnonymousPosting_ShippableHelper_UpdaterCore
 * @version 1
...
class AnonymousPosting_ShippableHelper_UpdaterCore
{
    ...
    {
        if (isset($GLOBALS[AnonymousPosting_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']])) {
            if (XenForo_Application::debugMode()) {
                die(sprintf('Updater for %s has been setup by %s already!', $config['apiUrl'],
                    $GLOBALS[AnonymousPosting_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']]));
            }
            return;
        }
        $GLOBALS[AnonymousPosting_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']] = __METHOD__;
...
EOF;
        ...
        if (!empty($_GET[self::PARAM_DOWNLOAD])
            ...
            if (!empty($data[$addOnId]['links']['content'])) {
                $tempFile = AnonymousPosting_ShippableHelper_TempFile::download($data[$addOnId]['links']['content']);
    ...
    private static function _updateSelf(array $config)
    {
        ...
        $configOptionId = AnonymousPosting_ShippableHelper_Updater::getConfigOptionId($config['apiUrl']);
        ...
        if ($configVersion < AnonymousPosting_ShippableHelper_Updater::$_version) {
            ...
            $config['version'] = AnonymousPosting_ShippableHelper_Updater::$_version;
            $configUpdated[] = '$config["version"]';
        }

        $addOnIdsUpdated = false;
        foreach ($GLOBALS[AnonymousPosting_ShippableHelper_Updater::KEY][$config['apiUrl']]['addOns'] as $addOnId => $enabled) {
        ...
        if (count($configUpdated) > 0) {
            ...
            if (XenForo_Application::debugMode()) {
                XenForo_Helper_File::log(__CLASS__, sprintf('%s($apiUrl=%s): $configVersion=%d,'
                    ...
                    var_export(AnonymousPosting_ShippableHelper_Updater::$_version, true),
    ...
    {
        $configOptionId = AnonymousPosting_ShippableHelper_Updater::getConfigOptionId($apiUrl);
        $templateTitle = $configOptionId;
        $templateModKey = $configOptionId;
    ...
    private static function _refreshData($apiUrl, array $addOnIds, $forceRefresh)
    {
        ...
        if (empty($data)
            || $data['version'] < AnonymousPosting_ShippableHelper_Updater::$_version
            || $data['timestamp'] < XenForo_Application::$time - 86400
        ) {
            $data = array(
                'version' => AnonymousPosting_ShippableHelper_Updater::$_version,
                'timestamp' => time(),
    ...
    private static function _fetchData($url, array $addOnIds, $accessToken = '')
    {
        ...
        $url .= sprintf('%s_version=%d', ((strpos($url, '?') === false) ? '?' : '&'),
            AnonymousPosting_ShippableHelper_Updater::$_version);
    ...
    private static function _getCache($apiUrl)
    {
        ...
        $data = $dataRegistryModel->get(AnonymousPosting_ShippableHelper_Updater::KEY);
    ...
    private static function _setCache($apiUrl, array $thisData)
    {
        ...
        $dataRegistryModel->set(AnonymousPosting_ShippableHelper_Updater::KEY, $data);
        return true;
    }
